...
> **背景与策略调整**：为了实现并行开发，在 **Setu** 通用虚拟机（MoveVM）完全集成前，先行将 LND 集成到 **Sui 网络**。Sui 作为成熟的 MoveVM 运行环境，非常适合先行验证 Move 版本的闪电网络合约逻辑。原本在 Bitcoin 上的闪电网络脚本逻辑（如多签、HTLC、时间锁、违约惩罚）将直接通过 **MoveVM 合约** 实现。
>
> **目标**：通过先行适配 Sui，加速 LND 对对象账户模型和 MoveVM 的兼容性改造。一旦 Setu 的 MoveVM 环境就绪，相关合约逻辑将迁移至 Setu。
>
